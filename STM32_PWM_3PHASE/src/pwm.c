...
// переменные для трёхфазного синуса
...
unsigned short	angle = 0;									// текущий угол для построения синуса.

unsigned short step_PWM_1 = 0;								// номера элементов массива. для сдвига фаз на 120 градусов
...
unsigned short TMR_T 			= period;					//	4500 - 16kHz COMPL_MODE		// 360 - 100 kHz PP_MODE
															// Для PP_MODE:  TMR_T = (72 000 / ЧастотакГц) / 2
...
unsigned short get_frequency_pwm()
{
	...
	frequency = 72000/(TMR_T * 2);	// расчёт частоты
	return(frequency);
}
...
void TIM3_IRQHandler (void)									// обработчик таймера для расчёта частоты частоты синуса
{
...
/*
		...
		if (count_test < 5000)								// тестовая качеля для светика на плате
		{
			count_test ++;
		}
		else

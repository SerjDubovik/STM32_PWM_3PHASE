...
extern unsigned short	array_mb[100];						// массив для хранения принятых/переданных слов
				...
				if (flag_UART_in == 1)					// проверка флага начала приёма. истина когда пришёл первый байт.
					...
					switch (swith_modbas)
					{
						...
						case 2:
						{
							...
							USART1->DR = array_modbas_out[0];		// Отправляем первый байт из массива для отправки
